...
@pytest.mark.parametrize(
    ...
    [(G1_PROFILE, G1), (X1_PROFILE, X1), (TKS_IP_PROFILE, TksIp)],
)
def test_create_device_uses_resolved_profile(
    profile: DeviceProfile,
    expected_type: type[G1] | type[X1] | type[TksIp],
) -> None:
    ...
    if isinstance(device, TksIp):
        assert device.host == "device.local"
        assert device.timeout == TIMEOUT
    else:
        assert device.api.host == "device.local"
        assert device.api.timeout == TIMEOUT


def test_create_device_rejects_unknown_profiles() -> None:
    profile = DeviceProfile(
    ...
    target = ResolvedTarget(profile, "unknown.local", "admin", "secret", TIMEOUT)
